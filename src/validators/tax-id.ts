...
const validateTaxId = (value: number | string | undefined): boolean => {
  ...
  if (!(/^\d{10}$/.test(checkValue))) {
    throw new Error('Номер РНОКПП має містити 10 цифр. Якщо особа відмовилася від отримання номеру або ще не отримала його, напишіть "Відсутній"')
  }
