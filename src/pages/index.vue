...
const downloadRais = async () => {
  ...
  for (const record of (results as Array<Record<string, unknown>>)) {
    ...
    if (record.category === '1') {
      ...
      const recentPcode = extractKatottg(record.admin4 as string ?? '')
      if (recentPcode !== null) {
        ...
        assistance.originStreet = record.recentStreet
      }
      const pcode = extractKatottg(record.recentAdmin4 as string ?? '') ?? recentPcode
      if (pcode !== null) {
        ...
        assistance.street = record.street
      }
